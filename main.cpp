...
double evaluateFunction(const std::string& expression, const std::vector<double>& point, mu::Parser& parser) {
    ...
    parser.ClearVar(); // Reset the variables in the parser for each point*/
...
std::pair<double, double> Montecarlo_integration(HyperSphere& hypersphere, int n, const std::string& function, int dimension) {
    ...
    for (int i = 0; i < n; ++i) {
        std::vector<double> random_point = hypersphere.generate_random_point(random_point_vector);
        random_point_vector.clear();
        if (!random_point.empty()) {
            result = evaluateFunction(function, random_point, parser);
            parser.ClearVar();
            total_value += result;

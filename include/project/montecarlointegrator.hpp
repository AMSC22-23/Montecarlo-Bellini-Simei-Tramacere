...
std::pair<double, double> Montecarlo_integration(int n, const std::string &function, HyperRectangle &hyperrectangle, 
    bool finance = false, const std::vector<const Asset*>& assetPtrs = std::vector<const Asset*>(), double std_dev_from_mean = 5.0, double* variance = nullptr,
    std::vector<double> vec = std::vector<double>(), int strike_price = 0);
